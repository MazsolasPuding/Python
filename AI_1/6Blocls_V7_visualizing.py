...
def rotaion_filter(seed):
    ...
    for i in range(3):
        ...
        # rotation[key] = translation_filter(rotation[key])
        print(len(rotation[key]))
    for n in range(len(seed)):
            if seed[n] in rotation["90_degree"]:
                pass
            else:
                result.append(seed[i])
    print(rotation["90_degree"])
...
all_connected = check_cube_connection(six_cubes)
...
#all_connected = check_cube_connection(less_than_four)

final_result = less_than_four
#create_plot_arrays(final_result)
...
rotaion_filter(final_result)

# final = []
# for i in range(len(final_result)):
#     final_result[i].sort(key=lambda row: (row[0], row[1], row[2]))
#     if final_result[i] in final:
#         pass
#     else:
#         final.append(final_result[i])
# print(len(final))
